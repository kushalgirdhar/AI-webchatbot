...
class E5HybridRetriever:
    """
    Combines BM25 keyword retrieval with multilingual-e5-base semantic retrieval
    from Qdrant using Reciprocal Rank Fusion (RRF) and a deterministic reranker.
    """
    ...
    def __init__(
        ...
        self.e5_retriever = (
            ...
            else E5Retriever(top_k=self.candidate_k)
        )
        self.reranker = reranker if reranker is not None else DeterministicReranker()

    def fuse_rrf(self, bm25_results, e5_results, rrf_k=None):
        """
        Fuse BM25 and E5 candidate rankings using Reciprocal Rank Fusion (RRF):
        ...
        # 2. Process E5 semantic candidates from Qdrant
    ...
    def search_candidates(self, query, candidate_k=None, rrf_k=None):
        ...
        e5_results = self.e5_retriever.search(query, top_k=k_candidates)

        fused = self.fuse_rrf(bm25_results, e5_results, rrf_k=rrf_k)
        return fused
    ...
    def search(
        ...
        1. Retrieve top candidates from BM25 and E5.
...
if __name__ == "__main__":
    ...
    retriever = E5HybridRetriever(chunks, top_k=5)
    ...
    for query in test_queries:
        ...
        print("\n--- Top 5 E5 Hybrid RRF Candidates ---")
        for rank, res in enumerate(rrf_candidates, start=1):
            ...
            print(
                ...
                f"| E5 rank: {res['qdrant_rank']} (score: {res['qdrant_score']:.4f})"
            )

        # 2. Final Reranked Results
        reranked_results = retriever.search(query, top_k=5, rerank=True)
        print("\n--- Top 5 E5 Hybrid RRF + Deterministic Reranker ---")
